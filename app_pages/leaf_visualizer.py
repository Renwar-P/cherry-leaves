...
def page_leaf_visualizer_body():
    ...
    if st.checkbox("Difference between average and variability image"):
      ...
      st.warning(
        f"* We notice that there is a difference between the average  "
        f" and the variablitiy image. However the shapes are recognizable.")
    ...
    if st.checkbox("Differences between average mildew infected and average uninfected leaves"):
          ...
          st.warning(
            ...
            f"leaves and those who are mildew infected." 
            f" The healthy leaves are clearly greener.")
          st.image(diff_between_avgs, caption='Difference between average images')

    if st.checkbox("Image Montage"): 
      st.write("* To refresh the montage, click on the 'Create Montage' button")
      my_data_dir = 'C:\Users\renwa\OneDrive\Desktop\Django3blog\cherry-leaves\inputs\cherry-leaves'
